...
시나리오 카드 목록(인트로 화면용 메타). 각 항목: `{ id, dir, title, subtitle, tag, stage, stars, status }`.

- `status`: `ready`(플레이 가능) | `planned`(카드만 표시, "준비 중" 배지)
- `tag`: 카드 훅 문구. 두 갈래 — **"기적을 재현하라"**(실제가 극적 승리인 경기), **"역사를 바꿔라"**(실제로 졌거나 비긴 경기)
- `stage`: 스테이지 번호(추천 플레이 순서, 인트로 정렬 기준). **잠금 아님** — 심사자·투표자가 어떤 경기든 바로 플레이할 수 있어야 하므로 하드 잠금은 두지 않는다. 클리어 기록(최고 점수·결과·승리 여부)은 localStorage(`planb-progress-v1`)에 저장되어 카드에 뱃지로 표시.
- `stars`: 난이도 표시(1~3). 1=입문(우루과이), 2=본편(포르투갈), 3=레전드(이탈리아·hard)
