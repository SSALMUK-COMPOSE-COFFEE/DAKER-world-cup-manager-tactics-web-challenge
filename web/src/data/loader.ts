...
export function loadScenarioIndex(): Promise<{ scenarios: ScenarioMeta[] }> {
  return fetchJson('/data/matches/index.json')
}

export function loadCommonComments(): Promise<CommentEntry[] | null> {
  return fetchJsonOptional('/data/comments/common.json')
}
...
export async function loadScenario(dir: string): Promise<{
  ...
  const [match, players, comments] = await Promise.all([
    fetchJson<MatchData>(`/data/matches/${dir}/match.json`),
    fetchJson<PlayersData>(`/data/matches/${dir}/players.json`),
    fetchJsonOptional<CommentEntry[]>(`/data/matches/${dir}/comments.json`),
  ])
